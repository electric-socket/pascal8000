...
OPNINPUT SR    R0,R0                   OPEN STD INPUT
         ...
         L     R14,=V(INDDNAM)         THEN TREAT AS OPEN EXTERNAL,
         MVC   BLKDDN,0(R14)           EXCEPT OPEN FILE BEFORE RETURN
         LR    RPARM,RBLK
         SPACE
OPENEXT  DS    0H
...
WRPUT0   DS    0H
         ...
         STH   RP,DCBLRECL             SET LRECL FOR OUTPUT
         B     WRPUT1
WRPUT05  DS    0H
...
*        INITIALISE SYSTEM CONSTANTS
         ...
         L     R15,=V(INTERUPT)
         DROP  R15
         SPIE (15),MF=(E,(5))          SET UP SPIE ENVIRONMENT
         USING PASDATA,R11
         ST    R1,OLDPICA
         BALR  R9,0
         USING *,R9
...
ENTRYVAR EQU   *-STACK                 VARIABLE PROC CALL ENTERS HERE
         L     R14,*-*(R15,R1)         LOAD PROC ADDRESS
...
*        2)    IF IN EXECUTION
*                 IF OLD PSW => COMPILED CODE AND #> 07FE (LONGJUMP)
...
TIMEX1   DS    0H
         ...
         C     R2,STAKHEAD             SEE IF IN CODE
         BC    10,TIMEX2               BR IF NOT
         C     R2,HEAD                 SEE IF IN CODE
         BC    4,TIMEX2                BR IF NOT
